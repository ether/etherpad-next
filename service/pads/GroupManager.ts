import { db, getDb } from '@/backend/DB';
...
export const listAllGroups = async () => {
  let groups = await (await getDb())!.get('groups');
  groups = groups || {};
...
export const deleteGroup = async (groupID: string): Promise<void> => {
  ...
  const {sessionIDs = {}} = await (await getDb())!.get(`group2sessions:${groupID}`) || {};
  await Promise.all(Object.keys(sessionIDs).map(async (sessionId) => {
    await deleteSession(sessionId);
  }));

  await Promise.all([
    (await getDb())!.remove(`group2sessions:${groupID}`),
    ...
    // @ts-ignore
    (await getDb())!.setSub('groups', [groupID], undefined),
  ...
  await (await getDb())!.remove(`group:${groupID}`);
};
...
export const createGroup = async () => {
  ...
  // @ts-ignore
  await (await getDb())!.set(`group:${groupID}`, {pads: {}, mappings: {}});
  ...
  // @ts-ignore
  await (await getDb())!.setSub('groups', [groupID], 1);
  return {groupID};
};
...
export const createGroupIfNotExistsFor = async (groupMapper: string|object) => {
  ...
  const groupID = await (await getDb())!.get(`mapper2group:${groupMapper}`);
  if (groupID && await doesGroupExist(groupID)) return {groupID};
  const result = await createGroup();
  await Promise.all([
    (await getDb())!.set(`mapper2group:${groupMapper}`, result.groupID),
    ...
    // @ts-ignore
    (await getDb())!.setSub(`group:${result.groupID}`, ['mappings', groupMapper], 1),
  ]);
  return result;
};
