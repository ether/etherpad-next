...
export class TextLinesMutator {
  private readonly _lines: string[]|ArrayLike<string>;
  private readonly _curSplice: any;
   ...
   * @returns {string[]}
   */
  _linesSlice(start: number, end: number): string[] {
   ...
   */
  skipLines(L: number, includeInSplice?: boolean) {
    if (!L) return;
    if (includeInSplice) {
